...
exports.addMemberChits = addMemberChits;
exports.deleteMemberChit = deleteMemberChit;
...
function mapMember(member, chits = []) {
    return {
        id: member.id,
...
async function findMemberChits(memberId) {
...
function normalizeChitGroupIds(body) {
...
async function syncMemberChits(connection, memberId, chitGroupIds) {
    await connection.execute('DELETE FROM member_chits WHERE member_id = ?', [memberId]);
    if (chitGroupIds.length) {
        await connection.query(`INSERT INTO member_chits (member_id, chit_group_id, join_date)
       VALUES ${chitGroupIds.map(() => '(?, ?, CURDATE())').join(', ')}`, chitGroupIds.flatMap((groupId) => [memberId, groupId]));
    }
    await connection.execute('UPDATE members SET group_id = ? WHERE id = ?', [chitGroupIds[0] || null, memberId]);
}
async function findMemberChitIds(memberId) {
    const [rows] = await db_1.pool.query('SELECT chit_group_id FROM member_chits WHERE member_id = ?', [memberId]);
    return rows.map((row) => row.chit_group_id);
}
async function notifyNewChits(memberId, chitGroupIds) {
    await Promise.all(chitGroupIds.map((groupId) => (0, notification_service_1.notifyMemberJoinedChit)(memberId, groupId)));
}
async function getMember(req, res) {
    ...
    return (0, response_1.ok)(res, mapMember(member, chits), 'Member loaded');
}
async function getMemberChits(req, res) {
    const member = await findMember(Number(req.params.id));
    if (!member)
        return (0, response_1.fail)(res, 404, 'Member not found');
    return (0, response_1.ok)(res, (await findMemberChits(member.id)).map(mapChit), 'Member chits loaded');
}
async function addMemberChits(req, res) {
    ...
    if (!chitGroupIds.length)
        return (0, response_1.fail)(res, 400, 'At least one chit group is required');
    ...
    await db_1.pool.query(`INSERT IGNORE INTO member_chits (member_id, chit_group_id, join_date)
     VALUES ${chitGroupIds.map(() => '(?, ?, CURDATE())').join(', ')}`, chitGroupIds.flatMap((groupId) => [memberId, groupId]));
    if (!member.group_id) {
        await db_1.pool.execute('UPDATE members SET group_id = ? WHERE id = ?', [chitGroupIds[0], memberId]);
    }
    await notifyNewChits(memberId, addedChitIds);
    return (0, response_1.ok)(res, (await findMemberChits(memberId)).map(mapChit), 'Member chits updated');
}
async function deleteMemberChit(req, res) {
    const memberId = Number(req.params.id);
    const chitGroupId = Number(req.params.chitGroupId);
    const member = await findMember(memberId);
    if (!member)
        return (0, response_1.fail)(res, 404, 'Member not found');
    await db_1.pool.execute('DELETE FROM member_chits WHERE member_id = ? AND chit_group_id = ?', [memberId, chitGroupId]);
    if (member.group_id === chitGroupId) {
        const [remaining] = await db_1.pool.query('SELECT chit_group_id FROM member_chits WHERE member_id = ? ORDER BY id LIMIT 1', [memberId]);
        await db_1.pool.execute('UPDATE members SET group_id = ? WHERE id = ?', [remaining[0]?.chit_group_id || null, memberId]);
    }
    return (0, response_1.ok)(res, { memberId, chitGroupId }, 'Member chit removed');
}
async function listMembers(req, res) {
    ...
    let chitsByMember = new Map();
    if (memberIds.length) {
        const [chits] = await db_1.pool.query(`SELECT mc.member_id, g.id, g.group_name AS name, g.monthly_amount, mc.join_date, mc.status
       FROM member_chits mc
       INNER JOIN chit_groups g ON g.id = mc.chit_group_id
       WHERE mc.member_id IN (${memberIds.map(() => '?').join(', ')})
       ORDER BY g.group_name`, memberIds);
        chitsByMember = chits.reduce((map, chit) => {
            map.set(chit.member_id, [...(map.get(chit.member_id) || []), chit]);
            return map;
        }, new Map());
    }
    return (0, response_1.ok)(res, { items: items.map((member) => mapMember(member, chitsByMember.get(member.id) || [])), total: Number(countRows[0]?.total || 0), page, limit }, 'Members loaded');
}
async function createMember(req, res) {
    ...
    if (!fullName || !mobileNumber) {
        return (0, response_1.fail)(res, 400, 'Name and phone are required');
    }
    const [existing] = await db_1.pool.query('SELECT id FROM members WHERE mobile = ? OR (? IS NOT NULL AND email = ?) LIMIT 1', [mobileNumber, email || null, email || null]);
    if (existing[0])
        return (0, response_1.fail)(res, 409, 'Member already exists. Edit the existing member to add chit groups.');
    const connection = await db_1.pool.getConnection();
    let insertedId = 0;
    try {
        await connection.beginTransaction();
        const [result] = await connection.execute(`INSERT INTO members (member_code, full_name, email, mobile, address, aadhaar_number, photo, group_id, status, joining_date)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, CURDATE())`, [
    ...
    await (0, activity_service_1.logRequestActivity)(req, 'member_added', `Member ${fullName} was added`, 'member', insertedId);
    const member = await findMember(insertedId);
    const chits = await findMemberChits(insertedId);
    return (0, response_1.created)(res, mapMember(member, chits), 'Member created');
}
async function updateMember(req, res) {
    ...
    await (0, activity_service_1.logRequestActivity)(req, 'member_updated', `Member ${name} was updated`, 'member', id);
    const member = await findMember(id);
    const chits = await findMemberChits(id);
    return (0, response_1.ok)(res, mapMember(member, chits), 'Member updated');
}
